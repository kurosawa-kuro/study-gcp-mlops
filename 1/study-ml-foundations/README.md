...
| パッケージ | 責務 |
|---|---|
| `pipelines/src/pipelines/housing_prices/` | データパイプライン + オーケストレーション (エントリーポイント) |
| `jobs/src/training/` | LightGBM 学習アルゴリズム |
| `jobs/src/training/evaluation/` | 精度評価 (RMSE, R²) + W&B 実験ログ |
| `app/src/app/` | FastAPI 推論 API + Jinja2 フロントエンド |
| `common/src/common/` | 共通定義 (特徴量カラム, 設定ベースクラス, ロギング, Run ID 生成) |

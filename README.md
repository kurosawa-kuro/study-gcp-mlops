...
- Phase 3 以降は不動産検索ドメインで **Local -> GCP -> Vertex AI -> GKE/KServe** へ展開
- Phase 3/4/5/6 は **LightGBM + multilingual-e5 + Meilisearch のハイブリッド構成を必須** とする
...
| 6 | `6/study-hybrid-search-gke/` | GKE/KServe 差分移行（Draft） | serving 層を GKE + KServe へ置換し、Phase 5 の学習/データ基盤を維持 | GKE, KServe, Gateway API, Workload Identity | uv + GKE/KServe |
...
| uv | Python 依存管理 | Phase 3 | Phase 3–6 |
...
- **GKE/KServe への serving 差分移行（Draft）**: Phase 6（前提: **Phase 5 の Vertex 構成理解**）
...
- Phase 個別入口:
  ...
  - `6/study-hybrid-search-gke/docs/02_移行ロードマップ.md`（Draft）

...
- Phase 6: Phase 5 と**同じ不動産ハイブリッド検索ドメインを維持**し、PMLE 試験範囲の追加技術 (BQML / Dataflow / Explainable AI / Monitoring SLO 等) と Cloud Composer 本線 orchestration を **論理境界として導入** (引き算チェーン上の Phase 5 → 6 が Composer 本線化境界)。実装本体は Phase 7 配下に集約 (引き算で Phase 6 へ派生)
- Phase 7: **教材コード完成版の到達ゴール / canonical 起点**。Cloud Composer 本線 orchestration + PMLE 4 技術 + Vertex AI Pipelines + **Vertex AI Feature Store (Feature Group / Feature View / Feature Online Store)** + **Vertex Vector Search** + Model Registry + BigQuery + Meilisearch + **GKE Deployment + KServe InferenceService (Vertex AI Endpoint からの差し替え)** をすべて本 phase 配下に本実装。引き算で Phase 6 / 5 / 4 / 3 / 2 / 1 を後方派生 (詳細は §4)。Feature Online Store は KServe から **Feature View 経由で** opt-in 参照 (Phase 7 固有)
...
|---|---|---|---|---|---|
| 1 | `1/study-ml-foundations/` | ML 基礎(回帰、引き算で派生) | preprocess / feature engineering / training / evaluation / artifact 出力 (model.pkl / metrics.json / params.yaml / `runs/{run_id}/`) | LightGBM, PostgreSQL | (実装データ削除済、docs のみ。`pre-phase1-impl-removal` tag で復元可) |
...
| 3 | `3/study-hybrid-search-local/` | 不動産ハイブリッド検索(Local、引き算で派生) | lexical + semantic + rerank、LambdaRank、RRF、Port/Adapter 実践 | Meilisearch, multilingual-e5, LightGBM LambdaRank, Redis, uv | (実装データ削除済、docs のみ。`pre-phase3to6-impl-removal` tag で復元可) |
| 4 | `4/study-hybrid-search-gcp/` | 不動産ハイブリッド検索(GCP、引き算で派生) | GCP マネージドサービス化、RRF、再学習ループ、IaC/CI、**BigQuery feature table / view の土台作成** (Phase 5 Feature Store の入力源)、**Secret Manager → Cloud Run secret injection(必須習得)** | Cloud Run, GCS, BigQuery, Cloud Logging, **Secret Manager**, **Pub/Sub, Eventarc, Cloud Scheduler, Artifact Registry, Cloud Build**, Terraform, WIF, GitHub Actions | (実装データ削除済、docs のみ) |
| 5 | `5/study-hybrid-search-vertex/` | Vertex AI 本番MLOps基盤移行 (引き算で派生) | Vertex Pipelines (KFP v2) / Endpoint / Model Registry / Monitoring / Dataform への adapter 差し替え。**Vertex AI Feature Store (Feature Group / Feature View / Feature Online Store) により training-serving skew を防ぐ特徴量管理を必須化** (Online Store を使う実務では Feature View が serving 接続点)。**Vertex Vector Search を ME5 ベクトル検索の本番 serving index として採用** (BigQuery 側は embedding 生成履歴・メタデータ保持、Phase 4 の BQ `VECTOR_SEARCH` は置換)。Dataform は BigQuery feature table / view / embedding metadata table の管理に利用。**Phase 4 の Cloud Scheduler + Eventarc + Cloud Function trigger は軽量 orchestration 経路として継続** し、Vertex Pipelines / Feature Store 更新 / Vector Search index 更新を必要最小限で接続する (Composer 本線化は引き算チェーン上の Phase 6 論理境界) | Vertex AI Pipelines, Vertex AI Endpoint, Vertex AI Model Registry, Vertex AI Model Monitoring, **Vertex AI Feature Store (Feature Group / Feature View / Feature Online Store)**, **Vertex Vector Search**, Dataform | (実装データ削除済、docs のみ) |
| 6 | `6/study-hybrid-search-pmle/` | GCP PMLE + 運用統合ラボ (引き算で派生) | 引き算チェーン上の論理境界として PMLE 範囲の追加技術 (BQML / Dataflow / Explainable AI / Monitoring SLO 等) と Cloud Composer 本線 orchestration が初登場。**実装本体は Phase 7 配下に集約**、Phase 6 は docs / 仕様書のみの派生 placeholder (引き算で Phase 7 から派生)。Phase 5 → 6 が Composer 本線化境界 (Vertex `PipelineJobSchedule` 完全撤去、Cloud Scheduler / Eventarc / Cloud Function trigger は軽量代替経路 / smoke / manual trigger 用途で残置)。不変はハイブリッド検索中核 (`/search` default) のみ | BQML, Dataflow (Apache Beam Flex Template), **Cloud Composer / Managed Airflow Gen 3 (本線 orchestration、引き算チェーンでは Phase 6 が論理境界)**, Monitoring SLO + burn-rate alert, TreeSHAP / Explainability, **Composer-managed BigQuery monitoring query** | (実装データ削除済、docs のみ) |
| 7 | `7/study-hybrid-search-gke/` | **教材コード完成版の到達ゴール / canonical 起点** | **Cloud Composer 本線 orchestration + PMLE 4 技術 + Vertex AI Pipelines + Feature Store + Vertex Vector Search + Model Registry + BigQuery + Meilisearch + GKE + KServe を本 phase 配下に本実装**。引き算で Phase 6 / 5 / 4 / 3 / 2 / 1 を後方派生する戦略 (§4)。Kubernetes 運用論点は抑え、まず動かす。SLO は `k8s_service`、TreeSHAP 用 explain 専用 Pod、**Feature Online Store (Feature View 経由) を KServe から参照する経路** を追加 | GKE Autopilot, KServe, **Cloud Composer / Managed Airflow Gen 3 (本実装 = canonical)**, Gateway API + HTTPRoute, External Secrets Operator, Workload Identity, GMP (PodMonitoring), HPA, IAP (GCPBackendPolicy), NetworkPolicy, Helm provider, **Vertex AI Feature Store (Feature Online Store / Feature View)** | uv + GKE Autopilot/KServe + Composer |
...
### Cloud Composer の位置づけ (Phase 7 で本実装、引き算で Phase 6 派生)

**要点** (canonical な詳細仕様は **Phase 7 [`docs/architecture/01_仕様と設計.md` §3](7/study-hybrid-search-gke/docs/architecture/01_仕様と設計.md)** を参照):

- **Phase 7 で本実装** (`7/study-hybrid-search-gke/infra/terraform/modules/composer/` + `7/study-hybrid-search-gke/pipeline/dags/` の 3 DAG = 教材コード完成版の到達ゴール)。引き算チェーン上では Phase 5 → 6 が Composer 本線化の **論理境界** (Phase 5 までは Cloud Scheduler + Eventarc + Cloud Function の軽量経路)
- **Composer (上位 orchestrator) × Vertex Pipelines (下位 ML executor) の上下関係** で運用 — 同責務を両層に持たせない
- **Vertex `PipelineJobSchedule` は完全撤去**、他の trigger (Cloud Scheduler / Eventarc / Cloud Function) は軽量代替・smoke 用途として残す (本線と二重起動はしない)
- **Phase 5 で Composer を導入しない理由**: Vertex AI 本番 MLOps 基盤 (Pipelines / Feature Store / Vector Search) の構築に集中させ、運用統合 (Composer DAG 化) は引き算チェーン上の Phase 6 論理境界に分離することで、コスト / 切り分け負荷を抑える

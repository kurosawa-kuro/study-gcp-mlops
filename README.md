...
- Phase 7 は **Optional / Advanced**。**Phase 6 の serving 層のみを GKE + KServe に置き換える Draft**（Pipelines / Feature Group / Model Registry / BigQuery / Meilisearch 等は Phase 6 から継承）。Run 2 で **Vector Search / Agent Builder の副経路は削除** (主経路 Meilisearch + BQ VECTOR_SEARCH と機能カニバリだったため)、SLO / Feature Online Store / Vertex CPR explain Pod を追加
...
| 8 | `8/study-hybrid-search-gitops/` | **Optional / 必須ではない** Kubernetes 運用 / GitOps ラボ | Phase 7 の構成を GKE Standard + Argo CD 管理へ発展。NodePool / autoscaling / GitOps / sync / rollback / drift 管理を学ぶ | GKE Standard, Argo CD, KServe, Gateway API, Workload Identity, Terraform | uv + GKE Standard + Argo CD |

### Phase 8 の非負制約（必須）

- Phase 7 の GKE/KServe serving 構成をベースにする
- ハイブリッド検索中核コードは変更しない
- 主題は ML/MLOps ではなく Kubernetes 運用 / GitOps とする
- GKE は Standard を使い、NodePool / autoscaling / taints / tolerations / resource request を学習対象に含める
- Argo CD を導入し、K8s manifest は Git を正とする
- 手動 `kubectl apply` は初期検証のみ許容し、最終形は Argo CD sync に寄せる
- Terraform はクラスタ・IAM・Argo CD 土台まで、アプリ配備は Argo CD 管理に分離する
...
- Phase 個別入口:
  ...
  - `6/study-hybrid-search-pmle/README.md`（PMLE 8 技術を Phase 5 実コードに実統合、2026-04-24 完了）
  - `6/study-hybrid-search-pmle/docs/01_仕様と設計.md`（統合トピック詳細 + ファイル配置図）
  - `6/study-hybrid-search-pmle/docs/02_移行ロードマップ.md`（決定的仕様、8 トピック全完）

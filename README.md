...
MLOps 学習用の 5 フェーズ構成リポジトリ（Phase 2 は新設予定 — 現 Phase 1 から app / pipeline / Port-Adapter を切り出す枠）。全 phase を単一の親 Git リポジトリで管理する。
...
| 2 | （新設予定） | （TBD） | App (API 連携) / パイプライン / Port-Adapter 等デザインパターン導入（現 Phase 1 から切り出し） | （TBD） |
| 3 | `3/study-hybrid-search-local/` | 不動産検索（Local） | ハイブリッド検索アーキテクチャの理解（Port/Adapter 設計 / スコア融合 / ランキング学習） | LightGBM **LambdaRank** + multilingual-e5 + Meilisearch + PostgreSQL + Redis |
| 4 | `4/study-hybrid-search-cloud/` | 不動産検索（GCP） | Cloud Native 化と MLOps パイプライン（IaC / CI/CD / 監視 / 再学習ループ） | Cloud Run + BigQuery（`VECTOR_SEARCH`）+ Dataform + **RRF** + Terraform + WIF |
| 5 | `5/study-hybrid-search-vertex/` | 不動産検索（Vertex AI） | Vertex AI プリミティブへの「差分工事」移行（adapter 差し替え） | Vertex Pipelines (KFP) / Endpoints × 2 / Model Registry / Feature Group / Monitoring v2 / Vizier |

**ドメイン統一の経緯:** 構想段階では「社内規定検索（検証）→ 商品検索（実務）」の転用案もあったが、Phase 3 → 4 の移植の学びに集中するため、両 Phase とも不動産検索でドメイン統一した（2026-04-20 確定）。

各 Phase の詳細な習得要素と Phase 間の差分（残す / 変える / 加える / やめる）は下記セクションを参照。

Dockerfile 配置の共通ルールは `docs/05_Docker配置規約.md` を参照。
...
### Phase 3: ハイブリッド検索 Local（`3/study-hybrid-search-local/docs/教育資料/`）

動画タイトル「ハイブリッド検索入門」。Phase 1 の ML 基礎を前提に、「検索システムの中で ML をどう使うか」へ視点転換するのがゴール。

**Phase 1 → 3 で変わる項目（stack 差分表）**
| 軸 | Phase 1 | Phase 3 |
|---|---|---|
...
### Phase 4: ハイブリッド検索 GCP（`4/study-hybrid-search-cloud/docs/教育資料/`）

動画タイトル「GCP-ML入門 — Cloud Run + BigQuery で学ぶ MLOps パイプライン」。Phase 3 の検索思想を保ったまま実行基盤を GCP に置き換え、「MLOps = 工程の連結」を掴むのがゴール。**Vertex AI は使わない**軽量 MLOps。
...
- 候補取得 / 再ランキングの責務分離
- **LightGBM LambdaRank + NDCG 監視**（Phase 3 で導入済、目的関数は変わらず）
- `multilingual-e5` の `query:` / `passage:` prefix 規約
- モデル未配置時のフォールバック可用性（Phase 4 では **rerank-free MVP** として `final_rank = lexical_rank`）
- `candidate_limit` vs `limit` の責務分離
- Port/Adapter パターン（本 Phase で **AST/parity テスト**により強制検査まで拡張）
- 行動ログ設計の考え方（テーブル名とパイプ経路が変わるだけで意図は同じ）

**Phase 4 で新規登場**
- **RRF（Reciprocal Rank Fusion）** — lexical rank と semantic rank の順位ベース融合。スコアスケール差に頑健（Phase 3 の特徴量化方式からの置換）
...
- 再学習ループ 3 経路の基礎形（Scheduler + Eventarc、Phase 5 で 3 経路に拡張）
- Workload Identity Federation（GitHub Actions → GCP、SA Key 禁止）
- 非負制約 / 権威順位 / feature parity invariant の運用ドキュメント体系

**Phase 4 でやめる / 登場しないもの**
- PostgreSQL（DB として。Phase 3 のローカル DB はもう使わない）
...
### Phase 5: ハイブリッド検索 Vertex AI（`5/study-hybrid-search-vertex/docs/教育資料/`）

動画タイトル「GCP-ML Vertex 編 — Phase 4 に Vertex AI レイヤを後付けする」。Phase 1〜4 受講済みを前提に、**Phase 4 のコア（検索 / 特徴量 / 学習ロジック）を残したまま、Port/Adapter の adapter だけ Vertex 実装に差し替える「差分工事」** がゴール。

**Phase 4 → 5 で変わる項目（stack 差分表）**
| 軸 | Phase 4 | Phase 5 |
|---|---|---|
...
**Phase 4 から変えない項目（Vertex に相当物なし / 移行 ROI 低 / 既存運用の保全）**
...
**Port/Adapter 境界の維持（Phase 5 の核心）**
...
- `encoder_server.py`: FastAPI、`AIP_HEALTH_ROUTE` / `AIP_PREDICT_ROUTE` / `AIP_HTTP_PORT` 準拠。起動時 `AIP_STORAGE_URI`（GCS）から ME5 checkpoint を download → `E5Encoder.load`。推論ロジックは `common/embeddings/e5_encoder.py` をそのまま import（Phase 4 と同一）
...
Phase 4 の 5 ファイル lockstep（schema / feature_engineering / infra `ranking_log` RECORD / monitoring SQL / Dataform SQLX）に、**6 番目として `infra/modules/vertex/main.tf::google_vertex_ai_feature_group_feature` × 7 列** を追加。`tests/parity/test_feature_parity_feature_group.py` が `FEATURE_COLS_RANKER` との 1:1 対応を検査。クエリ時算出の 3 列（`me5_score` / `lexical_rank` / `semantic_rank`）は Feature Group 非登録（Phase 4 の監視 SQL 除外規則と同じ）。
...
**9 Service Account 分離（Phase 4 の 5 SA から +4）**
...
- VECTOR INDEX は provider 未対応、手動 DDL 必須（Phase 4 から継続）

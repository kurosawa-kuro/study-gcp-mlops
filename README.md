...
| 6 | `6/study-gcp-ml-engineer-cert/` | GCP PMLE 試験範囲を Phase 5 実コードに実統合 (2026-04-24 全 8 完了) | T1-T8 の 8 技術を adapter / 副経路 / 追加エンドポイント / 追加 Terraform モジュールとして Phase 5 コードに実装。不変はハイブリッド検索中核 (`/search` default) のみ、default flag では Phase 5 挙動完全維持 | BQML / Dataflow / Vertex Vector Search / Explainable AI (TreeSHAP) / Cloud Monitoring SLO / Gemini RAG / Agent Builder (Discovery Engine) / Model Garden enrichment | uv + Vertex AI + Terraform (Phase 5 継承) |

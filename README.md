...
- Phase 6 は **Phase 5 と同じ不動産ハイブリッド検索ドメインを題材として維持**し、Phase 5 実装を比較基準として **GCP Professional Machine Learning Engineer 認定相当の総仕上げ** を行う。成果物は docs + `labs/` のみで、**Phase 5 コード (`app/` / `ml/` / `pipeline/` / `infra/` 等) はなるべく変えない**（不要な改変は Phase 5 の完成系を崩すダメージになる）
...
| 6 | `6/study-gcp-ml-engineer-cert/` | GCP ML Engineer 認定相当の総仕上げ（Phase 5 継承ドメイン） | PMLE 試験範囲の整理、Phase 5 実装を基準にした設計判断、MLOps、Responsible AI、GenAI/RAG、監視、コスト・運用判断。Phase 5 コードはなるべく変えない | Vertex AI, BigQuery ML, Dataflow, Model Garden, Agent Builder, Vertex Vector Search, Monitoring | ドキュメント + `labs/` 小規模ハンズオン |
...
- **Phase 5 コード (`app/` / `ml/` / `pipeline/` / `infra/` / `scripts/` / `tests/` / `monitoring/` / `env/` / `tools/` / `pyproject.toml` / `uv.lock` / `Makefile` / `.github/`) はなるべく変えない**。Phase 5 は 231 tests / 6 ファイル parity invariant / `make check` / `make deploy-all` ↔ `make destroy-all` の整合が取れた完成系で、不用意な改変は Phase 5 の教育価値を毀損する
- **新規ハンズオンは `labs/<topic>/` 配下に隔離する**（Phase 5 コードへの import / 継承は避け、読み取り参照のみ）
- **Phase 5 状態の GCP リソース (既存 Endpoint / Pipeline / Registry / BQ テーブル) には原則書き込まない**。新規リソースは `labs-<topic>-` プレフィックスで別名化し、手順末尾で削除する
- 改変が避けられないと判断された場合のみ、事前に明示的な合意を必要とする
...
| uv | Python 依存管理 | Phase 3 | Phase 3–5 / 7（Phase 6 は labs でのみ使用） |
...
- **GCP ML Engineer 認定相当の総仕上げ**: Phase 6（前提: **Phase 4/5 の GCP / Vertex 構成理解**。Phase 5 実装を比較基準として活用し、Phase 5 コードはなるべく変えない）
...
- Phase 6 (PMLE 総仕上げ) でも **Phase 5 実装を具体ドメインのケーススタディとして活用** する方が、抽象トピック暗記より試験対策として有利。Responsible AI / Fairness が「住宅差別」の実題材で学べる、GenAI / RAG が既存ハイブリッド検索の自然な拡張になる、という学習上のメリットもある

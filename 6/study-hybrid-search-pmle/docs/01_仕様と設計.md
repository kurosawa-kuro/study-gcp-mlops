...
- **継承 (Phase 6 からそのまま)**: PMLE 4 技術 (T1 BQML / T2 Dataflow / T4 Explainable AI / T5 Monitoring SLO) + 補助 (Feature Group / Online Store)、Vertex AI Pipelines (`property-search-embed` / `property-search-train`)、Vertex Model Registry、BigQuery データ層、Meilisearch (Cloud Run)
- **禁止 (親 README §1 教材対象外 / `docs/02_移行ロードマップ.md §1.4` 全 Phase 禁止)**: Vertex Vector Search / Matching Engine、Agent Builder / Discovery Engine、Gemini RAG (`/rag` 系)、Model Garden (`properties_enriched` 系)、Vertex AI Vizier、W&B、Looker Studio、Doppler。adapter / TF / ops script / settings / Make target / docs から全面排除済
...
本リポでは Phase 6 から PMLE 4 技術 (T1 BQML / T2 Dataflow / T4 Explainable AI / T5 Monitoring SLO) と補助 (Feature Group / Online Store) を継承する。Vertex Vector Search / Agent Builder / Gemini RAG / Model Garden / Vertex AI Vizier / W&B / Looker Studio / Doppler は親リポ README §1 教材対象外 + `docs/02_移行ロードマップ.md §1.4` 全 Phase 禁止技術リストに従い**一切扱わない**。1 行サマリ:
...
**補助 (Phase 7)**: Vertex AI Feature Online Store + FeatureView (`infra/terraform/modules/vertex/main.tf` 内、`enable_feature_online_store` フラグ)。`scripts/ops/vertex/feature_group.py` が regional public endpoint 経由で fetch する。

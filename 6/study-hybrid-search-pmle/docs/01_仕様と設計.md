...
- **不変 (絶対に変えない)**: 題材 (不動産ハイブリッド検索) と中核コード (Meilisearch BM25 + **Vertex Vector Search** + ME5 + RRF + LightGBM LambdaRank の挙動 / データフロー / デフォルトの `/search` 応答)。2026-05-01 改定で BQ `VECTOR_SEARCH` から Vertex Vector Search へ仕様置換 (Phase 5 起点、本 Phase で継承)
...
- **継承 (Phase 6 からそのまま)**: PMLE 4 技術 (T1 BQML / T2 Dataflow / T4 Explainable AI / T5 Monitoring SLO)、Vertex AI Pipelines (`property-search-embed` / `property-search-train`)、**Vertex AI Feature Store + Feature Group + Feature Online Store** (Phase 5 必須として導入されたものを継承)、**Vertex Vector Search** (ME5 ベクトル検索の本番 serving index、source は BigQuery 側 embedding テーブル)、Vertex Model Registry、BigQuery データ層 (analytics + embedding / 特徴量正本)、Meilisearch (Cloud Run)
- **禁止 (親 README §1 教材対象外 / `docs/02_移行ロードマップ.md §1.4` 全 Phase 禁止)**: Agent Builder / Discovery Engine、Gemini RAG (`/rag` 系)、Model Garden (`properties_enriched` 系)、Vertex AI Vizier、W&B、Looker Studio、Doppler。adapter / TF / ops script / settings / Make target / docs から全面排除済(Vertex Vector Search は 2026-05-01 改定で禁止リストから外し採用へ)
...
- **reference architecture**: Phase 5 [`docs/01_仕様と設計.md` §「実案件想定の reference architecture」](../../../5/study-hybrid-search-vertex/docs/01_仕様と設計.md) を継承(Elasticsearch + Redis 同義語辞書 + ME5 + Vertex Vector Search + LightGBM)。本リポは学習教材として Meilisearch + Redis cache を据え置く
...
- **PMLE 4 技術 (T1 BQML / T2 Dataflow / T4 Explainable AI / T5 Monitoring SLO)**: 実装ファイル位置・flag・Terraform モジュールは Phase 6 と同一
...
本リポでは Phase 6 から PMLE 4 技術 (T1 BQML / T2 Dataflow / T4 Explainable AI / T5 Monitoring SLO) と、Phase 5 必須として導入された **Vertex AI Feature Store + Feature Group + Feature Online Store** を継承する。Agent Builder / Gemini RAG / Model Garden / Vertex AI Vizier / W&B / Looker Studio / Doppler は親リポ README §1 教材対象外 + `docs/02_移行ロードマップ.md §1.4` 全 Phase 禁止技術リストに従い**一切扱わない**(Vertex Vector Search は 2026-05-01 改定で禁止リストから外し、Phase 5 から継承する semantic 検索の本番 serving index として採用。embedding 生成履歴・メタデータの正本は BigQuery 側に残す)。1 行サマリ:

| # | PMLE 技術 | 継承ファイル | 扱い |
|---|---|---|---|
| T1 | BQML popularity | `app/services/adapters/bqml_popularity_scorer.py`、`scripts/bqml/train_popularity.sql` | そのまま。`make bqml-train-popularity` で学習 |
| T2 | Dataflow streaming | `ml/streaming/`、`infra/terraform/modules/streaming/` | そのまま。Flex Template Job は opt-in |
...
| T5 | Monitoring SLO | `infra/terraform/modules/slo/` | `service_type` を `k8s_service` で GKE `search-api` に attach |

**Feature Store (Phase 5 必須として継承)**: Vertex AI Feature Store + Feature Group + Feature Online Store + FeatureView (`infra/terraform/modules/vertex/main.tf` 内、`enable_feature_online_store` フラグ)。`scripts/ops/vertex/feature_group.py` が regional public endpoint 経由で fetch する。Phase 6 では本 Feature Store を据え置き、入力源側 (Dataflow streaming + Scheduled Query) を強化する。

...
本リポでは Phase 6 から PMLE 4 技術 (T1 BQML / T2 Dataflow / T4 Explainable AI / T5 Monitoring SLO) と、Phase 5 必須として導入された **Vertex AI Feature Store (Feature Group / Feature View / Feature Online Store)** を継承する。Agent Builder / Gemini RAG / Model Garden / Vertex AI Vizier / W&B / Looker Studio / Doppler は親リポ README §1 教材対象外 + `docs/02_移行ロードマップ.md §1.4` 全 Phase 禁止技術リストに従い**一切扱わない**(Vertex Vector Search は 2026-05-01 改定で禁止リストから外し、Phase 5 から継承する semantic 検索の本番 serving index として採用。embedding 生成履歴・メタデータの正本は BigQuery 側に残す)。1 行サマリ:

...
> 注記: 旧構成の履歴説明が一部残るが、実装の正は現行ツリー (`app/`, `ml/{common,data,training,serving}`, `pipeline/`, `scripts/local/`)。
...
- **BigQuery + GKE + KServe + Vertex AI Pipelines 中心**。モデル成果物は GCS (`gs://mlops-dev-a-models/lgbm/{date}/{run_id}/model.txt`) と Vertex Model Registry、系譜は BQ テーブル `mlops.training_runs`。**Vertex AI Pipelines / Feature Group / Model Registry は Phase 5 から継承、Serving 層のみ GKE + KServe に差し替え**
...
- **Phase 6 rerank-free MVP** — Booster ロード前でも `/search` は候補抽出結果 (`final_rank = lexical_rank`) を返せる。ステージング疎通条件は `docs/04_運用.md §1` の E2E STEP
...
| IaC | Terraform 1.9+ | |
| GKE Autopilot | `asia-northeast1` リージョナル、Gateway API + Workload Identity 既定有効、deletion_protection は dev=false / prd=true | Phase 6 serving 層の実行基盤。node-pool 管理を持たない学習設計 |
| search-api Deployment | `requests=500m/1Gi`, `limits=2/2Gi`, `minReplicas=1 maxReplicas=10` (HPA CPU 70% / Mem 80%) | Phase 5 の Cloud Run `--cpu 2 --memory 2Gi --min-instances 1 --max-instances 10` を GKE 等価に写像 |
| KServe 認可境界 | NetworkPolicy で search namespace → kserve-inference namespace の in-cluster アクセスのみ許可。外部 Gateway は作らない | Cloud Run `--no-allow-unauthenticated` 相当を再現 |
| search-api 外部公開 | Gateway API (`gke-l7-global-external-managed`) + IAP (`BackendConfig`) + Workload Identity | Cloud Run IAM-gated 相当。`sa-scheduler` のみ `/jobs/check-retrain` を呼べるように IAP allowlist |
| Service Account | 9 SA 分離 (`sa-api` / `sa-pipeline` / `sa-job-train` / `sa-job-embed` / `sa-endpoint-encoder` / `sa-endpoint-reranker` / `sa-pipeline-trigger` / `sa-dataform` / `sa-scheduler`) + WIF 専用 `sa-github-deployer` = 計 10 SA | 最小権限の境界、統合しない。Phase 5 の GSA をそのまま維持し、Phase 6 では **GKE KSA (search/search-api, kserve-inference/encoder, kserve-inference/reranker) に Workload Identity で bind** して使い回す |
...
KServe `property-encoder` の `minReplicas 1 → 0` に落とすとコスト削減するが ME5 モデルロードで数秒〜数十秒のコールドスタートが発生する。下げる際は必ず User に flag。

Phase 6 の予測分布 drift 検知は縮退 (Vertex Model Monitoring v2 は Vertex Endpoint 前提のため KServe 化で失う)。Training-Serving Skew 監視 (`monitoring/validate_feature_skew.sql`) は BigQuery 側検知なので継続動作する。詳細は `docs/02_移行ロードマップ.md §5.5` と §8。
...
| `make kube-creds` | 対象 GKE Autopilot クラスタの kubeconfig を取得 (`gcloud container clusters get-credentials`) |
| `make deploy-api` | CI を経由せず Cloud Build (`cloudbuild.api.yaml`) → Artifact Registry → `kubectl set image deployment/search-api` で local から rollout |
| `make deploy-kserve-models` | Vertex Model Registry の `production` alias artifact URI を引いて、KServe `InferenceService` の `storageUri` を `kubectl patch` で書き換える |
...
- **scripts/ の責務分割** (lifecycle 別、`scripts/README.md` に Python-default ルール明記):
  - `scripts/setup/` (doctor / tf_bootstrap / tf_init / tf_plan)、`scripts/deploy/` (**api_gke** / **kserve_models** / training_job_local)、`scripts/config/` (sync_dataform)、`scripts/checks/` (layers)、`scripts/ops/` (livez_check / search_check / ranking_check / feedback_check / training_label_seed / check_retrain)、`scripts/sql/` (BQ クエリ)
...
- KServe `InferenceService` の `storageUri` は `scripts/local/deploy/kserve_models.py` が Vertex Model Registry の `production` alias artifact を引いて `kubectl patch` で書き換える。Manifest には placeholder URI を残し、rollout の正本は Registry に置く

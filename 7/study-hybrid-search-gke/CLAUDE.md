...
本リポジトリ (`7/study-hybrid-search-gke`) で作業する Claude Code 向けのガイド。**Phase 7 は Phase 6 (`6/study-hybrid-search-pmle/`) の PMLE 技術統合 (BQML / Dataflow / Monitoring SLO + 補助 Explainable AI) と、Phase 5 必須の Vertex AI Feature Store (Feature Group / Feature View / Feature Online Store) / Vertex Vector Search をそのまま継承した上で、Serving 層のみを Vertex AI Endpoint から GKE + KServe InferenceService に差し替えた到達ゴール**。中核コード (不動産ハイブリッド検索) は変えず、encoder / reranker の推論呼び出しを cluster-local HTTP に切り替える。Phase 7 固有の追加として **KServe pod から Vertex AI Feature Online Store を opt-in 参照する経路** を持つ。
...
- **Phase 6 PMLE 技術 (BQML / Dataflow / Monitoring SLO、補助: Explainable AI) を完全継承**。Phase 5 必須の **Vertex AI Feature Store (Feature Group / Feature View / Feature Online Store)** と **Vertex Vector Search** (semantic 検索の本番 serving index、BQ `VECTOR_SEARCH` 経路は撤去) は Phase 5 → 6 → 7 と継承。**Cloud Composer / Managed Airflow orchestration は Phase 6 で本線昇格 + PMLE DAG 増設** したものを Phase 7 で継承 (Phase 5 では Cloud Scheduler / Eventarc / Cloud Function 軽量経路を本線として使う、Composer 未導入)。Phase 7 では Cloud Scheduler / Eventarc / Cloud Function trigger / Vertex `PipelineJobSchedule` の再導入は禁止 (orchestration 二重化禁止)。Serving 層のみ GKE + KServe に差し替え。Phase 7 固有: KServe から Feature Online Store を opt-in で参照する経路を追加
...
| **ハイブリッド検索中核** | Meilisearch BM25 (lexical) + **Vertex Vector Search** + multilingual-e5 (semantic) + RRF 融合 + LightGBM LambdaRank (rerank) の挙動 / データフロー / デフォルト `/search` レスポンス(2026-05-01 改定で BQ `VECTOR_SEARCH` から Vertex Vector Search に置換 — Phase 5 起点。仕様 docs 先行、実装は別 PR で追従。Composer による orchestration 本線化は Phase 6 起点) |

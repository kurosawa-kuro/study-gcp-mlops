...
本リポジトリ (`7/study-hybrid-search-gke`) で作業する Claude Code 向けのガイド。**Phase 7 は Phase 6 (`6/study-hybrid-search-pmle/`) の PMLE 技術統合 (BQML / Dataflow / Monitoring SLO + 補助: Explainable AI / Feature Group) をそのまま継承した上で、Serving 層のみを Vertex AI Endpoint から GKE + KServe InferenceService に差し替えた到達ゴール**。中核コード (不動産ハイブリッド検索) は変えず、encoder / reranker の推論呼び出しを cluster-local HTTP に切り替える。
...
- **Phase 6 PMLE 技術 (BQML / Dataflow / Monitoring SLO、補助: Explainable AI / Feature Group) を完全継承**。Serving 層のみ GKE + KServe に差し替え
...
- **Phase 6 PMLE 技術はそのまま**: BQML popularity, Monitoring SLO, Dataflow streaming, Explainable AI, Feature Group は Phase 6 と同一実装。Vertex Endpoint 部分だけ KServe に差し替え済み

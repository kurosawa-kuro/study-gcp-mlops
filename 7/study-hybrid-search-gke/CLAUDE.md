...
- **Phase 6 PMLE 技術 (BQML / Dataflow / Monitoring SLO、補助: Explainable AI) を完全継承**。Phase 5 必須の **Vertex AI Feature Store (Feature Group / Feature View / Feature Online Store)** と **Vertex Vector Search** (semantic 検索の本番 serving index、BQ `VECTOR_SEARCH` 経路は撤去) と **Cloud Composer / Managed Airflow orchestration** (Phase 5 起点で本線、Phase 6 で PMLE DAG 増設) も Phase 5 → 6 → 7 と継承。Cloud Scheduler / Eventarc / Cloud Function trigger / Vertex `PipelineJobSchedule` の再導入は禁止 (orchestration 二重化禁止)。Serving 層のみ GKE + KServe に差し替え。Phase 7 固有: KServe から Feature Online Store を opt-in で参照する経路を追加

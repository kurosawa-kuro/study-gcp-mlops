...
- **題材**: 自由文クエリ + フィルタ → 物件ランキング上位 20 件。3 段構成 = (1a) Meilisearch BM25、(1b) **Vertex Vector Search** (ME5 ベクトル正本ストア + ANN 検索)、(2) RRF 融合、(3) LightGBM `lambdarank` 再ランク
- **Phase 6 PMLE 技術 (BQML / Dataflow / Monitoring SLO、補助: Explainable AI) を完全継承**。Phase 5 必須の **Vertex AI Feature Store + Feature Group + Feature Online Store** と **Vertex Vector Search** (semantic 正本ストア、BQ `VECTOR_SEARCH` 経路は撤去) も Phase 5 → 6 → 7 と継承。Serving 層のみ GKE + KServe に差し替え。Phase 7 固有: KServe から Feature Online Store を opt-in で参照する経路を追加

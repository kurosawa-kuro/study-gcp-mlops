...
- **Phase 6 PMLE 技術 (BQML / Dataflow / Monitoring SLO、補助: Explainable AI) を完全継承**。Phase 5 必須の **Vertex AI Feature Store + Feature Group + Feature Online Store** と **Vertex Vector Search** (semantic 検索の本番 serving index、BQ `VECTOR_SEARCH` 経路は撤去) も Phase 5 → 6 → 7 と継承。Serving 層のみ GKE + KServe に差し替え。Phase 7 固有: KServe から Feature Online Store を opt-in で参照する経路を追加

...
### 4.7 Cloud Composer 継承確認 (Phase 6 起点 → Phase 7 継承)

- [ ] `infra/terraform/modules/composer/` (**Phase 6 で実装**) を Phase 7 dev 環境にも apply (`environments/dev/main.tf` で `module "composer"` を呼ぶ — Phase 6 から copy、Phase 7 はそのまま継承)
- [ ] `pipeline/dags/{daily_feature_refresh,retrain_orchestration,monitoring_validation}.py` の Phase 7 環境への deploy (`composer-deploy-dags` make target)。Phase 6 で増設した PMLE step (Dataflow / BQML / drift / Explainability) も含む
- [ ] Phase 4-5 までの **撤去対象を本線として再導入していない** ことを確認 (CI 検証。infra リソース自体は軽量代替 / smoke 用途で残置可、本線 trigger からは外れていること):

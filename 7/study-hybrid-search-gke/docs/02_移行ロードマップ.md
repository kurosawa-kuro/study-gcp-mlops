...
**Wave 2 で確認済みの補足**:
- `infra/manifests/kserve/reranker.yaml` の env vehicle 追加は **不要**。`KSERVE_FEATURE_ONLINE_URL` 廃止に合わせて search-api ConfigMap 経由へ一本化し、PR-4 docstring の予告は close
- `scripts/ci/sync_configmap.py` は **未追従**。`configmap.example.yaml` 側に追加済の Wave 2 キーを generator がまだ再現できないため、W2-5-c は未完了のまま残す
...
- [x] **W2-5-a**: `infra/manifests/search-api/configmap.example.yaml` に新 env vehicle を追加 — Wave 1 PR-1〜PR-4 で導入した env (default off なので値は空文字 / `bq` 維持):
...
- [x] **W2-5-b**: `infra/manifests/search-api/deployment.yaml` の env で上記を ConfigMap から参照 (default off になるよう値は空文字)
- [ ] **W2-5-c**: ConfigMap の値は §4.1 W2-1 / W2-2 の Terraform outputs から `scripts.setup.deploy_all` 経由で render (Phase 6 で確立済の wiring を踏襲)
- [x] **PR-4 deferred 解消**: `infra/manifests/kserve/reranker.yaml` の env vehicle は **`KSERVE_FEATURE_ONLINE_URL` 廃止により不要** → search-api ConfigMap 経由で完結。原本記述は不要、Wave 1 PR-4 docstring の予告は close

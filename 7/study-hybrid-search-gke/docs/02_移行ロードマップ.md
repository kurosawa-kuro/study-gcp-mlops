...
| **Wave 2** | **GCP インフラ層 (= クラウド側の主作業計画)** | ⏳ 着手前 | §4 が母艦。**実施順序 W2-1〜W2-9** (§4.0 表): VVS Terraform → FOS default flip + Feature View provision → IAM/WI bind → Composer 継承 (Phase 6 から copy) → Manifest env → backfill → smoke → default flip → 負債解消。**Phase 6 起点の Composer は新規実装せず継承のみ** (Phase 7 固有経路 = KServe → FOS の **Feature View 経由** opt-in 参照) |
...
>
> **位置付け**: 本セクションは **クラウド側 (GCP インフラ) の修正作業計画の母艦**。親 [`README.md`](../../../README.md) は教育設計、本 phase [`docs/01_仕様と設計.md`](01_仕様と設計.md) は仕様 canonical、本セクションが **「いつ何を Terraform / kubectl / gcloud で叩くか」の作業計画** を持つ。
...
| **W2-5** | Manifest env vehicle 追加 (search-api ConfigMap に `SEMANTIC_BACKEND` / `FEATURE_FETCHER_BACKEND` / VVS / FOS 関連 env を default off で追加) | §4.3 | W2-1 / W2-2 | env 投入準備完了 (default off なので挙動不変) |
...
| **W2-8** | default flip (ConfigMap default を `vertex_vector_search` / `online_store` に書き換え、PMLE 4 技術 + Composer-managed BQ monitoring query が DAG 経由で動作確認) | §4.6 | W2-7 + 1 週間 dev 安定 | M6 達成 (search-api default が VVS / FOS 経路、本線 retrain は Composer DAG) |
...
- [ ] **W2-1**: **`infra/terraform/modules/vector_search/`** を実装 (Wave 1 PR-1 が `app/services/adapters/vertex_vector_search_semantic_search.py` を空 endpoint で先行追加済、本ステップで実 resource を provision)
...
- [ ] **W2-1**: `infra/terraform/environments/dev/main.tf` で `module "vector_search"` を有効化 + 出力を `module.gke` (KServe SA bind 用) / search-api ConfigMap (W2-5) / `infra/terraform/modules/composer/` (Phase 6 から継承、DAG param 用) へ伝搬

**既存 Terraform の設定変更**:

- [ ] **W2-2**: `infra/terraform/modules/vertex/variables.tf::enable_feature_online_store` の default を `true` に変更 (`mlops-dev-a` PDCA 都合は `terraform.tfvars` で override 可)。**Feature View** (`property_features` を source とする View) も同モジュールで provision、Feature View ID と regional public endpoint URL を outputs に追加 (Wave 1 PR-2 が `vertex_feature_view_id` / `vertex_feature_online_store_endpoint` を settings に予約済)
...
- [ ] `infra/terraform/modules/iam/main.tf` の現状確認 — `sa-api` には既に `roles/aiplatform.user` 付与済 ✓、`sa-pipeline` / `sa-pipeline-trigger` も同様 ✓
...
- [ ] **新規 (W2-3-b)**: KServe encoder / reranker KSA (`kserve-inference/encoder`, `kserve-inference/reranker`) → 専用 GCP SA に WI bind し、Vertex Vector Search query / Feature View read 権限付与。**Phase 7 固有経路**: KServe pod から Feature Online Store を **Feature View 経由で** opt-in 参照する経路 (本 phase docs/01 §0) のため
...
- [ ] **W2-5-a**: `infra/manifests/search-api/configmap.example.yaml` に新 env vehicle を追加 — Wave 1 PR-1〜PR-4 で導入した env (default off なので値は空文字 / `bq` 維持):
...
- [ ] **W2-5-b**: `infra/manifests/search-api/deployment.yaml` の env で上記を ConfigMap から参照 (default off になるよう値は空文字)
- [ ] **W2-5-c**: ConfigMap の値は §4.1 W2-1 / W2-2 の Terraform outputs から `scripts.setup.deploy_all` 経由で render (Phase 6 で確立済の wiring を踏襲)
- [ ] **PR-4 deferred 解消**: `infra/manifests/kserve/reranker.yaml` の env vehicle は **`KSERVE_FEATURE_ONLINE_URL` 廃止により不要** → search-api ConfigMap 経由で完結。原本記述は不要、Wave 1 PR-4 docstring の予告は close

### 4.4 ops スクリプト / one-off (W2-6 / W2-7)

- [ ] **W2-6**: `scripts/setup/backfill_vector_search_index.py` (初回 backfill 用 one-off。`feature_mart.property_embeddings` 全行を読み出し → `MatchingEngineIndex.upsert_datapoints` で push、batch size = 500 = Wave 1 PR-3 の `vector_search_upsert_batch_size` default と一致)
- [ ] **W2-7-a**: `scripts/ops/vertex/vector_search.py` (smoke 用、`find_neighbors` を直接叩いて top-K を表示)
...
- [ ] **W2-7-c**: `tests/integration/parity/test_semantic_backend_parity.py` (BQ vs VVS 上位 K diff) と `test_feature_fetcher_parity.py` (BQ vs FOS feature 値 diff) を live GCP marker 付きで実装 (Wave 1 で skip 予約済)
...
### 4.6 default flip + 検証 (W2-8 = M6 達成)

`SEMANTIC_BACKEND` / `FEATURE_FETCHER_BACKEND` の default を Vertex AI 経路へ flip する手順:

- [ ] live で `SEMANTIC_BACKEND=vertex_vector_search` を 1 週間 dev で動かす (`mlops.ranking_log` の latency / NDCG diff を観測、Cloud Monitoring で alert 設定)
- [ ] OK なら ConfigMap default を `vertex_vector_search` に flip (= **M6 達成**)
- [ ] 同様に `FEATURE_FETCHER_BACKEND=online_store` を 1 週間 dev で動かす (Feature View 経由で BQ direct fetch と feature 値が一致することを `tests/integration/parity/test_feature_fetcher_parity.py` で daily 検証) → flip
...
- [ ] **default flip 後**: 本 phase [docs/01_仕様と設計.md](01_仕様と設計.md) §4.5 表を「`vertex_vector_search` / `online_store` が default」に更新 (Wave 3 で実施)
...
- [ ] **mypy pre-existing 負債** (PR-1 audit で発見): `app/services/search_service.py:218` (3) / `app/services/adapters/lexical_search.py:57,63` (2) / `app/api/routers/ops_router.py:99,118-120` (4) / `tests/conftest.py:251` (1)。**合計 9 件**、PR-4 では触れず
- [ ] **`tests/integration/parity/`** に live GCP 比較 test を追加 (Wave 1 で skip した `test_semantic_backend_parity.py` / `test_feature_fetcher_parity.py`、§4.4 W2-7-c と統合)

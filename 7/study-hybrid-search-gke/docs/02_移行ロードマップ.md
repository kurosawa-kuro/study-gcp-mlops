...
| PR-4 | KServe Reranker → Feature Online Store opt-in 参照 (Phase 7 固有) | `FEATURE_FETCHER_BACKEND=online_store` と `vertex_feature_online_store_*` 設定時のみ fresh feature 取得が有効化される |
...
| PR-4 KServe → FOS opt-in | adapter / settings / manifest env 追加 — 全てローカル可 (manifest apply は Wave 2) | unit test で `FEATURE_FETCHER_BACKEND=online_store` 分岐を確認 | live search-api / KServe 経路での opt-in 参照 smoke |
...
| 操作 | パス | 役割 |
|---|---|---|
| 編集 | `app/services/adapters/kserve_reranker.py` | predict 前に `FeatureFetcher` を opt-in で呼ぶ (`FEATURE_FETCHER_BACKEND=online_store` group で制御) |

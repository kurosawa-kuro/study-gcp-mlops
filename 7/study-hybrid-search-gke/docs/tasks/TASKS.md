...
| **V5** | Composer DAG `retrain_orchestration::check_retrain` SUCCEEDED | ✅ **DONE** (検証ゲート) | Run 4: **check_retrain = success**（F5 有効）。同一 Run の DAG Run **全体は failed**（`submit_train_pipeline` → failed、下流 upstream_failed）— §0.1 のゲートは **check_retrain のみ** で満たす |
...
= **V5 検証ゲート (check_retrain) は Run 4 で達成** (F1–F5 適用 + F5 hardcode で env 伝播タイミングを排除)。後続 task（submit / wait / promote）は本番運用の別境界。
...
| **V5 後続（任意）** | 同一 DAG Run の `submit_train_pipeline` 以降が **success** まで完走 | ゲートは `check_retrain` のみ達成済。Vertex / pipeline 権限・待機は別観測 |
...
| **Wave 2** | GCP インフラ層 (クラウド側主作業) | ✅ **罰金回避ライン達成** (V1+V2+V5 `check_retrain`) | offline 部 ✅、live 部: deploy-all / run-all-core / Composer **`check_retrain` task success** (Run 4)。残: V3 destroy / V4 2 周目 import |
...
**✅ 罰金回避ライン (2026-05-03 夜)**:
- [x] **V1** `make deploy-all` 完走
- [x] **V2** `make run-all-core` 完走
- [x] **V5** `retrain_orchestration::check_retrain` task **success** (Run 4、F1–F5 反映)

**⏳ 明日以降**:
- [ ] V3: `make destroy-all` live 1 周 verify (state rm + import pattern)
- [ ] V4: 2 周目 deploy-all で `terraform import` 経路の live 検証
- [ ] V5 追跡（任意）: 同一 DAG Run の Vertex submit / wait 完走、または新 trigger で E2E 確認
- [ ] V6: `tests/integration/parity/*` の `live_gcp` 本実行
...
- [x] Composer DAG **`check_retrain` 本線** — `retrain_orchestration` task **success** (V5 Run 4、§0.1 ゲート)
- [ ] `/search` semantic 経路が Vertex Vector Search 1 本 (BQ fallback 撤去後) — V2 で実機確認済、継続モニタ
- [ ] feature 取得経路が Feature Online Store (Feature View 経由) 1 本 (BQ direct fetch 撤去後) — 同上

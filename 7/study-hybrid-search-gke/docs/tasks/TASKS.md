...
| V5-7b Run 4 | `composer-deploy-dags` (修正版 `_pod.py` upload) + 90s wait + DAG trigger | 🔄 **進行中** (background task `bj0shc32y`) | tf-apply / image rebuild 不要 (env_variables 変化なし、image 変化なし、`_pod.py` GCS upload のみ) |
| V5-7c Run 4 | DAG smoke → **SUCCEEDED 確認** | ⏳ | F1〜F5 全適用後、真因可視化済の試行 |
...
= **V5 ゴール到達まで残り ~15 分** (image rebuild + tf-apply 並行 → composer-deploy-dags → smoke retry)。Run 3 は SUCCEEDED が想定確率 70%、失敗時も DIAG log で次の真因が即座に見える設計。

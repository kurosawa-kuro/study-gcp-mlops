...
> 本ドキュメントは Phase 7 の仕様と設計、**および Phase 7 がゴールであることに伴い、ハイブリッド検索 Phase 横断仕様 (§2) と Cloud Composer の位置づけ (§3) の canonical** を持つ。権威順位は `02_移行ロードマップ.md > 01 > README.md`。親リポ [`docs/01_仕様と設計.md`](../../../../docs/architecture/01_仕様と設計.md) は本ファイルを参照する薄いナビゲーションハブ (重複設定回避)。
...
>
> **Phase 7 = canonical 起点**: 教材コード完成版は Phase 7 が持ち、引き算で Phase 6 を派生させる関係 (親 README §4 「基本戦略：「引き算」によるPhase間コード生成」)。Composer は本 phase で本実装する。

Phase 7 ゴールから引き算で後方 Phase を派生する戦略との整合のため、Cloud Composer / Managed Airflow Gen 3 は **Phase 7 で本線オーケストレーターとして本実装** する。教育文脈での説明では「**Phase 6 が Composer 初登場 (引き算チェーン上)**」と表現するが、**実装はすべて Phase 7 配下** (`infra/terraform/modules/composer/` / `pipeline/dags/` / `scripts/deploy/composer_deploy_dags.py` / `make composer-deploy-dags`)。Phase 5 は Vertex AI 本番 MLOps 基盤の構築 — Pipelines / Feature Store / Vector Search / Model Registry / Monitoring — に集中するため Composer を持たず、Phase 4 軽量経路 (Cloud Scheduler + Eventarc + Cloud Function (Gen2)) を本線として継続する。**Phase 5 → 6 が Composer 本線化の引き算境界** (引き算チェーン上の論理境界。Phase 6 配下にも実装が必要なわけではない)。
...
|---|---|---|---|
| **Orchestration control plane** | **Cloud Composer DAG** | **上位** (司令塔) | **Phase 7 で本実装** (実装は本 phase 配下、引き算チェーンでは Phase 6 が Composer 初登場の論理境界。Phase 5 までは Cloud Scheduler / Eventarc / Cloud Function 軽量経路を本線として使う) |
| ML pipeline execution plane | Vertex AI Pipelines | **下位** (Composer から submit される) | Phase 5 起点 (Phase 5 では本線 trigger は Cloud Scheduler 経由、Phase 6 引き算境界で Composer 経由に切替) |
| Streaming / batch transform | Dataflow Flex Template | 下位 (Composer DAG が起動) | Phase 6 引き算境界 |
...
### 3.3 Composer が引き受ける orchestration 責務 (Phase 7 で本実装、引き算で Phase 6 に派生)

本線 retrain schedule の集約。Phase 4-5 までの軽量経路は引き算後の Phase 6 / 本 phase でも軽量代替として残す:
...
### 3.4 残す (Phase 7 = canonical / 引き算後の Phase 6 でも継続)
...
- **Phase 4-5 までの Cloud Scheduler / Eventarc / Cloud Function (Gen2) リソース自体** — 撤去せず、軽量代替・比較対象 / smoke / manual trigger 用途として保持 (= 「Composer が常に正解」とは限らない設計判断力を残すため)。**ただし本線 retrain schedule と同じ job を別系統で起動しないこと** (= 二重起動禁止、§3.6 カニバリ NG)

### 3.5 Phase 5 内部マイルストーン (5A-5D) / Phase 6 内部マイルストーン (6A-6B) と DAG 増設の段差

Phase 5 は Vertex AI 本番MLOps基盤の構築に集中させ、引き算チェーン上の Phase 6 論理境界で Composer 本線昇格 + PMLE 統合を表現する (実装は Phase 7 = canonical に集約、引き算で Phase 6 に派生)。Phase 番号は増やさず、各 Phase docs / Issue / commit メッセージでラベルとして使う。
...
**Phase 6 (6A-6B) — Cloud Composer 本線昇格 + PMLE 追加技術統合** (実装は Phase 7 = canonical に集約):

| ID | スコープ | 主成果物 (実体は Phase 7 配下) |
|---|---|---|
| **6A** | Composer DAG 本線昇格 | `7/study-hybrid-search-gke/infra/terraform/modules/composer/`、`7/study-hybrid-search-gke/pipeline/dags/{daily_feature_refresh,retrain_orchestration,monitoring_validation}.py` の 3 DAG、Vertex `PipelineJobSchedule` 完全撤去、Cloud Scheduler / Eventarc / Cloud Function trigger を本線から軽量代替へ格下げ |
...
| Phase 6 (引き算境界、論理上の起点 = 6A の DAG / 6B の PMLE 増設は Phase 7 から派生) | `daily_feature_refresh` (Dataform + Feature Store sync) / `retrain_orchestration` (Vertex training pipeline submit) / `monitoring_validation` (skew/drift + SLO + burn-rate) — 6B で Dataflow / BQML / Explainability を増設 | 3〜5 本 |
| Phase 7 (canonical = 本実装) | 上記 3 DAG を本 phase で本実装。serving 差分は KServe 側で完結し、orchestration 層には新 DAG を追加しない | 同上 |
...
- ❌ 引き算で Phase 6 を派生する際、Phase 6 側が Composer を捨てて Cloud Scheduler に戻る先祖帰り (Phase 6 は **Phase 7 = canonical から GKE/KServe 等を引き算した結果** であり、Composer は引き算対象に含めない)
- ❌ Phase 5 で Composer を導入してしまう (Phase 5 は Vertex AI 本番MLOps基盤の構築に集中、Composer 本線化は Phase 6 の引き算境界)
...
Phase 7 のコストで知りたいのは、**どの課金サービスが関わるか** と **3 時間の学習セッションでいくらかかるか** である。前提は「Phase 7 full 構成を 3 時間起動し、`search-api` build 1 回、Vertex Pipelines 学習 job 1 回、通常量の search / log / monitoring を流し、学習終了後に `destroy-all`」。

| 区分 | 課金サービス | Phase 7 での用途 | 3h 概算 (JPY) |
|---|---|---|---|
| orchestration | Cloud Composer | DAG orchestration | **¥380 前後** |
| serving | GKE Autopilot | `search-api` / KServe 推論 Pod の実行基盤 | **¥100-140** |
| serving | Gateway / Load Balancer | GKE 入口 | **¥20-40** |
| serving | Vertex Vector Search | semantic 候補取得の serving index | **¥150-240** |
| serving | Vertex AI Feature Store / Feature Online Store | online feature fetch | **¥25-70** |
| serving | Cloud Run | Meilisearch runtime | **¥10-20** |
| ML 実行 | Vertex AI Pipelines | train / eval / register | **¥15-30** |
| build | Cloud Build | `search-api` image build | **¥10 前後** |

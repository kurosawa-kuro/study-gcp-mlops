...
| ML コンポーネント再利用 | 強い | 普通 |
| コスト | run 課金中心。いずれも **GCP / Vertex 技術学習が目的** であり、**実運用は前提にしない**。学習当日は `deploy-all -> run-all -> destroy-all` の 1 cycle で閉じ、常設しない前提 | 環境常駐ぶん相対的に重い (DCU-hour 課金)。同じく **学習用途・当日 `destroy-all` 前提** だが、1 cycle ~¥190、1 日 ~¥920 の目安で、destroy 漏れリスクが特に大きい。詳細 [docs/runbook/05_運用.md §1.4-bis](../runbook/05_運用.md) |

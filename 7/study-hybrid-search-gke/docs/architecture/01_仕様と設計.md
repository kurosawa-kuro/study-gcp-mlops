...
>
> **2026-05-03 教育コード化方針の改訂**: コード化対象を **Phase 1 / 2 / 3 / 4 / 7** に限定。Phase 5 / 6 は **論理 Phase** (個別コード保守なし、Phase 7 完成版コードを参照する資料中心の Phase)。Phase 4 は **Phase 7 完成版からの引き算ではなく Phase 3 からの足し算** で独立コード化。詳細は [`../../../../docs/architecture/教育コード化についての仕様変更.md`](../../../../docs/architecture/教育コード化についての仕様変更.md)。
...
## 3. Cloud Composer の位置づけ (Phase 7 で本実装 = canonical、Phase 6 が論理境界)
...
### 3.6 Phase 7 (= canonical) / 論理境界の Phase 6 で禁止する状態 (カニバリ / 二重化 / 先祖帰り)
...
1. `docs/tasks/TASKS_ROADMAP.md §0` で **Phase 6 (論理境界) と何が違うか** (= serving 層のみ) を押さえる。教育コード化方針 (Phase 1/2/3/4/7 = コード化、Phase 5/6 = 論理 Phase) は親 [`docs/architecture/教育コード化についての仕様変更.md`](../../../../docs/architecture/教育コード化についての仕様変更.md) を参照

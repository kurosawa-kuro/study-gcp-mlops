...
| [`04_運用.md`](./04_運用.md) | **「やる」**: PDCA メインフロー (`make deploy-all` ↔ `make destroy-all`) + 個別 STEP 1–17 詳細 + 定常運用 + インシデント対応 + ロールバック | 新規参加者 / 学習目的の PDCA 担当 / 運用担当 | PDCA 周回手順・閾値・アラート変更時 |
| [`05_検証.md`](./05_検証.md) | **「OK か判定する」**: ローカル検証 (L1-L5) + GCP 検証ゲート (G1-G6、特に G3 = 3 種 all non-zero) + 一過性エラー recovery + 性能ベースライン (退化検知) + 検証依頼プロンプト例 | 新規参加者 / Claude Code に依頼する人 / レビュアー | 検証ゲート / ベースライン値 / 新しい一過性エラーが見つかった時 |
...
| 検証ゲート / 性能ベースライン / 一過性エラー recovery の追加・更新 | `05_検証.md` (新ゲート → §2 or §3、新ベースライン → §5、新一過性エラー → §4) |
| ローカルで完結しない検証が増えた / 減った | `05_検証.md §2 L5` の理由 + `05_検証.md §3` のゲート定義 |

...
Phase 7 の主目的は **Phase 6 (`study-hybrid-search-pmle`) の PMLE 技術統合と、Phase 5 必須の Vertex AI Feature Store + Feature Group + Feature Online Store / Vertex Vector Search をそのまま継承した上で、Serving 層のみを Vertex AI Endpoint から GKE + KServe InferenceService に差し替え、動くコードとして手を動かして学ぶ** こと (Phase 7 は本リポジトリの到達ゴール、`make check` / `make tf-validate` で CI 同等チェック PASS)。Phase 7 固有として **KServe pod から Feature Online Store を opt-in 参照する経路** を追加。本ドキュメントは 02 の決定を前提に、**Phase 7 で変わる部分 (serving 層) と変わらない部分 (Pipelines / Vertex AI Feature Store + Feature Group + Feature Online Store / Vertex Vector Search / Model Registry / BigQuery / Meilisearch / Phase 6 PMLE 統合)** を設計レベルで載せる。
...
- **継承 (Phase 6 からそのまま)**: PMLE 4 技術 (T1 BQML / T2 Dataflow / T4 Explainable AI / T5 Monitoring SLO)、Vertex AI Pipelines (`property-search-embed` / `property-search-train`)、**Vertex Vector Search** (ME5 ベクトル検索の本番 serving index、source は BigQuery 側 embedding テーブル)、**Vertex AI Feature Store + Feature Group + Feature Online Store** (Phase 5 必須として導入されたものを継承)、Vertex Model Registry、BigQuery データ層 (analytics + embedding / 特徴量正本)、Meilisearch (Cloud Run)。Phase 7 固有として KServe から Feature Online Store を opt-in 参照する経路を追加
- **継承しない (Phase 6 限定 opt-in)**: **Cloud Composer / Managed Airflow Gen 3** は Phase 6 の optional orchestration lab であり、Phase 7 (GKE/KServe 到達ゴール) には混ぜない。Phase 7 の orchestration は Phase 5/6 の Cloud Scheduler + Eventarc + Cloud Function + Vertex Pipelines 本線をそのまま踏襲する (詳細は親 [`README.md` §「Cloud Composer の位置づけ」](../../../README.md))
...
- **reference architecture**: Phase 5 [`docs/01_仕様と設計.md` §「実案件想定の reference architecture」](../../../5/study-hybrid-search-vertex/docs/01_仕様と設計.md) を継承(Elasticsearch + Redis 同義語辞書 + ME5 + Vertex Vector Search + LightGBM)。本リポは学習教材として Meilisearch + Redis cache を据え置き、KServe 経路でも同構成
- **前提知識**: Phase 5 (Vertex AI プリミティブ + **Vertex AI Feature Store + Feature Group + Feature Online Store** 必須 + Vertex Vector Search 必須) + Phase 6 (PMLE 統合: BQML / Dataflow / Monitoring SLO + 補助 Explainable AI) を既習扱い

...
- **継承 (Phase 6 からそのまま)**: PMLE 6 技術 (T1 BQML / T2 Dataflow / T4 Explainable AI / T5 Monitoring SLO / T6 RAG (Gemini) / T8 Model Garden)、Vertex AI Pipelines (`property-search-embed` / `property-search-train`)、Vertex Feature Group + Online Store、Vertex Model Registry、BigQuery データ層、Meilisearch (Cloud Run)
- **✂️ 削除 (Phase 7 Run 2)**: T3 Vertex Vector Search / T7 Agent Builder — 主経路 (Meilisearch + BQ VECTOR_SEARCH) と機能カニバリだった副経路。adapter / TF / ops script / settings / Make target 全排除
...
## 3. Phase 6 PMLE 技術 (継承 / 削除)

Phase 7 では Phase 6 の adapter / 新規エンドポイント / Terraform モジュールを引き継ぎつつ、**主経路 (Meilisearch lexical + BQ VECTOR_SEARCH semantic) と機能カニバリだった副経路 2 件 (T3 / T7) を削除**した。詳細は [Phase 6 の `docs/01_仕様と設計.md §3`](../../../6/study-hybrid-search-pmle/docs/01_仕様と設計.md) を参照。1 行サマリ:
...
| ~~T3~~ | ~~Vertex Vector Search~~ | ✂️ **削除** (`app/services/adapters/vertex_vector_search_semantic.py` / `infra/terraform/modules/vector_search/` / `scripts/ops/vertex_vector_search.py` / `semantic_backend` setting) | semantic 主経路の BQ VECTOR_SEARCH と機能カニバリで邪魔だったため Run 2 で全削除。復活が必要な場合は git history 参照 |
| T4 | Explainable AI | `ml/serving/reranker.py` の `/explain` ルート (Phase 7 では kserve-inference namespace に独立 Pod として deploy) | **`/search?explain=true` で TreeSHAP attributions 取得**。KServe stock LightGBM v2 runtime は attribution 非対応なので Vertex CPR 互換 server を `KSERVE_RERANKER_EXPLAIN_URL` で配線 |
| T5 | Monitoring SLO | `infra/terraform/modules/slo/` | `service_type` を `cloud_run` → `k8s_service` に書き換えて GKE `search-api` に attach。Run 2 で apply 完走 |
| T6 | RAG (Gemini) | `app/services/adapters/gemini_generator.py`、`app/services/rag_summarizer.py`、`/rag` エンドポイント | そのまま。GKE Pod からも Vertex AI API を呼ぶだけなので serving 層差し替えの影響なし。Phase 7 では `gemini-2.5-flash` + `gemini_max_output_tokens=2048` がデフォルト |
| ~~T7~~ | ~~Agent Builder~~ | ✂️ **削除** (`app/services/adapters/agent_builder_lexical.py` / `infra/terraform/modules/agent_builder/` / `scripts/ops/vertex_agent_builder.py` / `lexical_backend` setting) | lexical 主経路の Meilisearch BM25 と機能カニバリで邪魔だったため Run 2 で全削除 |
...
**追加 (Phase 7)**: Vertex AI Feature Online Store + FeatureView (`infra/terraform/modules/vertex/main.tf` 内、`enable_feature_online_store` フラグ)。`scripts/ops/vertex_feature_group.py` が regional public endpoint 経由で fetch する。

**中核挙動への影響**: なし (主経路は Meilisearch + BQ VECTOR_SEARCH + LightGBM rerank で固定。削除した T3 / T7 は default OFF の opt-in 副経路だったので削除で機能損失ゼロ、Run 2 で `/search` 3-component と `/rag` `/search?explain=true` 全 PASS を確認済)。
...
│   ├── handlers/                 # 1 endpoint 1 file (各 < 40 行)
│   ├── mappers/                  # Pydantic ↔ domain 変換
│   ├── middleware/
│   └── routes/
└── services/
    ├── protocols/                # 1 Port 1 file
    ├── adapters/                 # 1 Adapter 1 file (production)
    ├── fakes/                    # Noop / InMemory (production-grade null impl)
    ├── search_service.py / feedback_service.py / rag_service.py / retrain_policy.py
...
├── ports/                        # data_job / evaluation_job / batch_serving_job は re-export
└── adapters/
```
...
- **Port (Protocol)**: `app/services/protocols/<name>.py`。1 Protocol 1 file。GCP SDK / lightgbm 等の concrete dependency を import 禁止
- **Adapter (production)**: `app/services/adapters/<name>.py`。1 Adapter 1 file。外部 SDK は adapter 内でのみ
- **Fake (production noop / in-memory)**: `app/services/fakes/<name>.py`。`Noop*` (feature flag disabled 時) と `InMemory*` (実装が未統合の段階で使う簡易版) の住処。test stub は別途 `tests/fakes/` (Phase F)
- **Service**: `app/services/<name>_service.py`。Port のみ import。HTTP 詳細を持たない
- **Handler**: `app/api/handlers/<endpoint>_handler.py`。`Depends` 注入で service を受け取り、Pydantic 型変換 (mapper) と HTTP ステータスだけ
- **Mapper**: `app/api/mappers/<endpoint>_mapper.py`。Pydantic schema ↔ domain dataclass の変換 1 箇所
- **ML / Pipeline Port-Adapter**: `ml/<feature>/{ports,adapters}/`、`pipeline/<verb>_job/{ports,adapters}/`。SDK 直叩きを adapter に隔離

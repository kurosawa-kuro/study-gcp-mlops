...
要約: **Elasticsearch + Redis 同義語辞書 + multilingual-e5 + Vertex AI Vector Search + RRF + LightGBM LambdaRank**。本リポは Meilisearch + Redis cache を **学習用 substitute** として据え置き、Port/Adapter で `MeilisearchAdapter` ↔ `ElasticsearchAdapter` および `ResultCache` ↔ `SynonymExpander` の差し替えで上記 reference architecture に到達できる構造を維持する。Meilisearch を採用している理由は、実案件の Elasticsearch をそのまま教材へ持ち込むのではなく、**lexical 検索をローカルでも触れて検証できる軽量な検索エンジンを確保するため**でもある。Meilisearch を据え置く理由の全体像は親 [`README.md` §「検索エンジン (Phase 3 以降): Meilisearch」](../../../README.md) を参照。

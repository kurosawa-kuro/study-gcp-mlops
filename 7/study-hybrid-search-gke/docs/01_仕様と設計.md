...
> 本ドキュメントは Phase 7 (`study-hybrid-search-gke`) の仕様と設計、**および Phase 7 がゴールであることに伴い、ハイブリッド検索の Phase 横断仕様 (Phase 3-7 共通) と Cloud Composer の位置づけ (Phase 6-7 共通) の canonical** を持つ。決定的仕様は `docs/02_移行ロードマップ.md` (権威順位: `02 > 01 > README.md`)。本ファイルが 02 と矛盾したら 02 を正として修正する。
>
...
- **継承 (Phase 6 からそのまま)**: PMLE 4 技術 (T1 BQML / T2 Dataflow / T4 Explainable AI / T5 Monitoring SLO)、Vertex AI Pipelines (`property-search-embed` / `property-search-train`)、**Vertex Vector Search** (ME5 ベクトル検索の本番 serving index、source は BigQuery 側 embedding テーブル)、**Vertex AI Feature Store (Feature Group / Feature View / Feature Online Store)** (Phase 5 必須として導入されたものを継承)、**Cloud Composer / Managed Airflow Gen 3 orchestration** (**Phase 6 で本線昇格** した DAG `daily_feature_refresh` / `retrain_orchestration` / `monitoring_validation` + PMLE 追加 step を、Phase 7 でもそのまま継承)、Vertex Model Registry、BigQuery データ層 (analytics + embedding / 特徴量正本)、Meilisearch (Cloud Run)。Phase 7 固有として KServe から Feature Online Store を opt-in 参照する経路を追加
...
- **Composer 二重化禁止**: Phase 7 は Phase 6 で本線昇格した Composer DAG をそのまま使い、Cloud Scheduler / Eventarc / Cloud Function trigger / Vertex `PipelineJobSchedule` を **再導入しない** (Phase 4-5 まで継続していた軽量経路の orchestration 責務は Phase 6 で Composer DAG に集約済。infra 自体は軽量代替・smoke / manual trigger 用途として残置可、ただし本線と同 job 二重起動は禁止) — 詳細は §3
...
- **前提知識**: Phase 5 (Vertex AI プリミティブ + **Vertex AI Feature Store (Feature Group / Feature View / Feature Online Store)** 必須 + Vertex Vector Search 必須、orchestration は Phase 4 軽量経路を継続) + Phase 6 (**Cloud Composer 本線昇格** + PMLE 統合: BQML / Dataflow / Monitoring SLO + 補助 Explainable AI) を既習扱い
...
| **6B** | PMLE 4 技術統合 | 上記 3 DAG に Dataflow Flex Template 起動 / BQML training / SLO + burn-rate 確認 / Explainability eval を増設 (or 新 DAG 追加) |

...
- **Fake (production noop / in-memory)**: `app/services/noop_adapters/<name>.py`。`Noop*` (feature flag disabled 時) と `InMemory*` (実装が未統合の段階で使う簡易版) の住処。test stub は別途 `tests/fakes/` (Phase F)

...
| drift 検知 | Vertex Model Monitoring v2 (Endpoint attach) | **self-managed Scheduled Query** (`monitoring/validate_model_output_drift.sql`) が `mlops.ranking_log` から `mlops.model_monitoring_alerts` を生成。Vertex Endpoint attach は縮退 |
...
| Vertex Model Monitoring v2 の縮退代替 | ✅ `monitoring/validate_model_output_drift.sql` + `model_output_drift_check` Scheduled Query で代替 |

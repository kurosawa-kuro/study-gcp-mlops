...
- **前提知識**: Phase 5 (Vertex AI プリミティブ + **Vertex AI Feature Store + Feature Group + Feature Online Store** 必須 + Vertex Vector Search 必須 + **Composer 本線 orchestration**) + Phase 6 (PMLE 統合: BQML / Dataflow / Monitoring SLO + 補助 Explainable AI) を既習扱い
...
| **5B** | Feature Store 必須化 | Vertex AI Feature Store + Feature Group + Feature Online Store の Terraform 定義 + `FeatureFetcher` Port + adapter (Phase 7 PR-2 完了) |
...
**Feature Store (Phase 5 必須として継承)**: Vertex AI Feature Store + Feature Group + Feature Online Store + FeatureView (`infra/terraform/modules/vertex/main.tf` 内、`enable_feature_online_store` フラグ)。`scripts/ops/vertex/feature_group.py` が regional public endpoint 経由で fetch する。Phase 7 固有として **KServe pod から Feature Online Store を opt-in 参照する経路** を追加 (既定では無効、`KSERVE_FEATURE_ONLINE_URL` 等で有効化)。

...
- **継承 (Phase 6 からそのまま)**: PMLE 4 技術 (T1 BQML / T2 Dataflow / T4 Explainable AI / T5 Monitoring SLO)、Vertex AI Pipelines (`property-search-embed` / `property-search-train`)、**Vertex Vector Search** (ME5 ベクトル正本ストア)、**Vertex AI Feature Store + Feature Group + Feature Online Store** (Phase 5 必須として導入されたものを継承)、Vertex Model Registry、BigQuery データ層 (analytics 用途)、Meilisearch (Cloud Run)。Phase 7 固有として KServe から Feature Online Store を opt-in 参照する経路を追加
- **教材対象外**: 親 [`README.md` §1](../../../README.md) のリストに従い、Phase 7 にも含めない (詳細は親 `docs/02_移行ロードマップ.md §1.4`)。Vertex Vector Search は 2026-05-01 改定で禁止リストから外し、Phase 5 から継承する semantic 正本ストアとして採用

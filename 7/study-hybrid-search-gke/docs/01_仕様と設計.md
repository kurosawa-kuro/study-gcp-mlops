...
- **継承 (Phase 6 からそのまま)**: PMLE 4 技術 (T1 BQML / T2 Dataflow / T4 Explainable AI / T5 Monitoring SLO)、Vertex AI Pipelines (`property-search-embed` / `property-search-train`)、Vertex Feature Group + Online Store、Vertex Model Registry、BigQuery データ層、Meilisearch (Cloud Run)
- **✂️ 削除 (親 README §1 教材対象外を反映)**: T3 Vertex Vector Search / T6 Gemini RAG / T7 Agent Builder / T8 Model Garden / Vizier — 主経路と機能カニバリ、もしくは学習価値が低いため、全 Phase 禁止。adapter / TF / ops script / settings / Make target / handler / schema / mapper / pipeline component すべて排除済
...
Phase 7 では Phase 6 の adapter / 新規エンドポイント / Terraform モジュールを引き継ぎつつ、親 README §1「教材対象外(全 Phase 禁止)」に該当する技術を削除する。詳細は [Phase 6 の `docs/01_仕様と設計.md §3`](../../../6/study-hybrid-search-pmle/docs/01_仕様と設計.md) を参照。1 行サマリ:
...
| ~~T3~~ | ~~Vertex Vector Search~~ | ✂️ **全 Phase 禁止** (機能カニバリ)。adapter / TF / ops script / settings / Make target すべて削除済 | 復活が必要な場合は git history 参照 |
...
| ~~T6~~ | ~~RAG (Gemini)~~ | ✂️ **全 Phase 禁止** (学習価値低)。`/rag` router / `RagService` / `RagSummarizer` / `GeminiGenerator` adapter / `Generator` Protocol / `app/schemas/rag.py` / `scripts/ops/vertex/rag.py` / `gemini_*` 設定 / Make target すべて削除済 | — |
| ~~T7~~ | ~~Agent Builder~~ | ✂️ **全 Phase 禁止** (機能カニバリ)。adapter / TF / ops script / settings / Make target すべて削除済 | — |
| ~~T8~~ | ~~Model Garden (Gemini enrichment)~~ | ✂️ **全 Phase 禁止** (Model Garden)。`scripts/enrichment/` / `feature_mart.properties_enriched` (BQ table + TF resource + output) / `make enrich-properties` すべて削除済 | — |
| ~~Vizier~~ | ~~Vertex AI Vizier~~ | ✂️ **全 Phase 禁止** (学習価値低)。`pipeline/training_job/components/vizier.py` / `enable_tuning` パラメータ / `_resolve_event_source` すべて削除済 | — |
...
**中核挙動への影響**: なし (主経路は Meilisearch + BQ VECTOR_SEARCH + LightGBM rerank で固定。削除した副経路は default OFF の opt-in だったため機能損失ゼロ)。

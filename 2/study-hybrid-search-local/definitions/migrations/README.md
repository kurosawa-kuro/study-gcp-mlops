...
`pipeline/batch/maintenance/run_migrations.py`（`pipeline.batch.maintenance.run_migrations`）経由で適用する。

例:

```bash
docker compose exec -T api python -m pipeline.batch.maintenance.run_migrations definitions/migrations/008_eval_and_kpi.sql
```

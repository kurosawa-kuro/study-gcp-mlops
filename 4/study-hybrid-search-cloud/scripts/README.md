...
scripts/
  ...
  dev/                               ← 環境前提・IaC・派生設定生成
    ...
    deploy_all.py                    ← `make deploy-all`
    destroy_all.py                   ← `make destroy-all`
    seed_minimal.py                  ← `make seed-test`
    seed_minimal_clean.py            ← `make seed-test-clean`
    sync_dataform.py                 ← `make sync-dataform-config`
  ci/                                ← 静的検査 (lint 系)
    check_layers.py                  ← `make check-layers`
  local/                             ← 手動 deploy・デプロイ後の runtime smoke / 検査
    deploy_api.py                    ← `make deploy-api-local`
    deploy_training_job.py           ← `make deploy-training-job-local`
...
| folder | 含めるもの | 含めないもの |
|---|---|---|
| `dev/` | 開発前提 / Terraform / 派生設定生成 / seed | デプロイ後の HTTP 検査 |
| `ci/` | repo 構造 / 命名 / 境界 / 静的解析 | 実行時の HTTP / DB 検査 |
| `local/` | デプロイ後の API / Job への HTTP / RPC 検査、手動 deploy | 構造検査 |
| `local/sql/` | 1 ファイル 1 SQL、`bq query` で叩く | Python ロジック |

`tests/` 側も同じ責務分割: `tests/unit/` / `tests/integration/` / `tests/e2e/`。

---

## Makefile 側の規約

- ターゲットは **1 行で script を呼ぶだけ** (`uv run python -m scripts.dev.X` / `uv run python -m scripts.local.X` / `bq query --project_id=$(PROJECT_ID) < scripts/local/sql/X.sql`)。
- Makefile に inline shell / heredoc / SQL define ブロックを書かない (出てきたら `scripts/` に移動)。
- ターゲット名と script ファイル名は対応させる (`make ops-livez` → `scripts/local/livez_check.py`)。
...
2. **置き場所を決める**:
   - 開発環境セットアップ / Terraform / 設定生成 → `scripts/dev/X.py`
   - デプロイ後の運用 / API smoke / 手動 deploy → `scripts/local/X.py`
...
- **本番ロジック**: `app/` / `ml/{embed,train,sync}/` / `common/` に置く。`scripts/` は thin wrapper のみ。
- **テスト**: `tests/` / `app/tests/` / `common/tests/` / `ml/{embed,train,sync}/tests/` に置く。
...
旧 shell 群は全て Python 化完了。さらに学習しやすい責務分割として `dev/` / `ci/` / `local/` に再編した。`scripts/` 直下と各サブフォルダは `__init__.py` を持つパッケージで、共通 helper は `scripts/_common.py` に集約 (env 取得 / gcloud subprocess / Cloud Run URL 解決 / OIDC token / IAM-gated http_json)。Makefile から `uv run python -m scripts.<folder>.<module>` で呼び出す。SQL ファイルは `scripts/local/sql/*.sql` に集約。`tests/` も `unit/` / `integration/` / `e2e/` に再編した。

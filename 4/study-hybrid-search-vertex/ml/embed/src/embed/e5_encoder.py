...
class E5Encoder:
    ...
    is not a direct dependency of ``embed``; tests can substitute an object
    ...
    def load(cls, *, model_dir: Path | None = None) -> E5Encoder:
        ...
        from HuggingFace on first use. In production the encoder CPR / embed
        pipeline hydrates the model from GCS into a temp dir first and passes
        that path, so there is no outbound network call at serve time.
        """

...
- Vertex AI Pipelines
- Vertex AI Endpoints
- **Vertex Vector Search** (ME5 ベクトルの正本ストア + ANN 検索)
- Model Registry
- **Vertex AI Feature Store + Feature Group + Feature Online Store(必須)** — training-serving skew 防止のため Phase 5 で必ず導入する。Phase 4 の BQ feature table / view を入力源とし、training (`pipeline/`) と serving (`app/`) の双方が同一 feature を読む
- Model Monitoring

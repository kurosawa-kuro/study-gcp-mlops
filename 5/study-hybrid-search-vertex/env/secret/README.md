...
本番 (Cloud Run) は **Secret Manager** 経由で環境変数に注入される（`.github/workflows/deploy-api.yml` の `--set-secrets` オプション参照）。
...
## Secret Manager への値投入手順

```bash
export MEILI_KEY="<Meilisearch の Master Key>"
...
3. 本番で必要なら Secret Manager に secret container を Terraform で作成し、deploy workflow の `--set-secrets` に追加

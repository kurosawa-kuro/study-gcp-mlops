...
- **Vertex Vector Search / Agent Builder / Vizier / Model Garden / Gemini RAG** は、ハイブリッド検索中核と機能カニバリを起こすか学習価値が低いため、全 Phase で導入・言及しない
...
- Phase 6: Phase 5 を継承し、追加技術は adapter / 副経路 / 追加エンドポイント / 追加 Terraform として統合
- Phase 7: Phase 6 から学習/データ基盤を継承し、serving 層のみ GKE + KServe に差し替える（Meilisearch master key は External Secrets Operator で GCP Secret Manager から K8s Secret へ自動同期）

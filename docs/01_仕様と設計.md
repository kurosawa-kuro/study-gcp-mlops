...
| 5 | `5/study-hybrid-search-vertex/` | Vertex AI 標準 MLOps 差分移行 | Vertex Pipelines（KFP v2） / Endpoint / Model Registry / Monitoring / Dataform への adapter 差し替え、**Feature Store / Feature Group / Feature Online Store による training-serving skew 防止を必須化**、**Vertex Vector Search を ME5 ベクトル検索の本番 serving index として採用**（BigQuery 側は embedding 生成履歴・メタデータ保持、Phase 4 の BigQuery `VECTOR_SEARCH` は置換） | Vertex AI Pipelines, Vertex AI Endpoint, **Vertex AI Feature Store, Vertex Feature Group, Vertex Feature Online Store**, **Vertex Vector Search**, Vertex AI Model Registry, Vertex AI Model Monitoring, Dataform, Cloud Function（Gen2） | uv + Vertex AI |
| 6 | `6/study-hybrid-search-pmle/` | GCP PMLE 追加技術ラボ（Phase 5 実コードへ統合） | PMLE 範囲の追加技術を adapter / 副経路 / 追加エンドポイント / Terraform として統合。default flag では Phase 5 挙動維持。**Feature Store は Phase 5 前提**とし、本 Phase では Dataflow / Scheduled Query / **Cloud Composer (opt-in)** による特徴量生成・更新・オーケストレーションを強化。不変はハイブリッド検索中核（`/search` default）のみ | BQML, Dataflow（Apache Beam Flex Template）, Monitoring SLO + burn-rate alert, TreeSHAP（Explainable AI）, Scheduled Query, **Feature Store 更新パイプライン**, **Cloud Composer / Managed Airflow Gen 3 (opt-in 副経路)** | uv + Vertex AI + Terraform |
...
## Cloud Composer の位置づけ (Phase 6 限定 opt-in)

Cloud Composer / Managed Airflow Gen 3 は **Phase 6 の optional orchestration lab** として扱う。Phase 5 / 7 には混ぜない (Phase 5 は Vertex 標準の主役、Phase 7 は GKE/KServe 到達ゴールに集中するため)。

| 項目 | 方針 |
|---|---|
| 位置付け | Phase 6 限定の **opt-in 副経路** |
| 本線 (default、維持) | Cloud Scheduler + Pub/Sub + Eventarc + Cloud Function (Gen2) + Vertex AI Pipelines |
| 副経路 (Composer DAG) | BigQuery freshness check / Dataform / Scheduled Query 起動 / Dataflow Flex Template 起動 / BQML training / Vertex Pipeline submit / 通知 — を 1〜2 本の DAG で束ねる比較教材 |
| Terraform | `enable_composer` flag で opt-in、default `false` (環境費用が発生するため) |
| やらない | 本線置換、全ジョブ Composer 化、複雑な DAG 設計、Phase 5 / Phase 7 への波及 |

学習価値: 「Composer を採用するかしないか」の **設計判断軸** を手元に持つこと。日次ジョブ程度なら Cloud Scheduler 経路の方が軽い、という結論を実体験できる構成。詳細は親 [`README.md` §「Cloud Composer の位置づけ」](../README.md) を参照。
...
- **Vertex Vector Search** は Phase 5 以降の semantic 検索の本番 serving index として採用する（実案件想定）。**embedding 生成履歴・メタデータの正本は BigQuery 側に保持し続ける**。Phase 4 のみ BigQuery `VECTOR_SEARCH` 経路を学習目的で残す
...
- Phase 5: Vertex Model Registry / Vertex Pipelines / Metadata / Vertex Endpoint / Vertex Model Monitoring / **Vertex AI Feature Store + Feature Group + Feature Online Store(training-serving skew 防止のため必須)** / **Vertex Vector Search(ME5 ベクトル検索の本番 serving index、embedding 履歴・メタデータは BigQuery 側に残す)**

...
- [ ] `meili-master-key` が Secret Manager に実在し、Cloud Run `--set-secrets` で注入される
- [ ] Meilisearch が master key auth で起動し、adapter が Authorization header を送る
- [ ] 全 Phase `pyproject.toml` から `wandb` が消え、`uv sync && make check` が通る
- [ ] Terraform `data` module に `wandb_api_key` / `doppler_token` resource が存在しない
- [ ] `.github/workflows/deploy-*.yml` に Doppler step が存在しない
- [ ] doc から「W&B」「Doppler」「Looker Studio」の実装手順記述がゼロ（設計履歴としての言及はこの節に集約）
- [ ] Phase 5 `pipeline/training_job/` から Vizier placeholder が消える

---

# Phase 6 Run 1 修正対策情報 (2026-04-24 整理)

Phase 6 `6/study-gcp-ml-engineer-cert` の動作検証 Run 1 (2026-04-24) で発生した不具合と対策をまとめる。詳細実行ログは [`動作検証結果.md §5`](./動作検証結果.md) を正本とし、本セクションは **「何が壊れていて、何を恒久的にどう直したか / 次どう予防するか」の index**。

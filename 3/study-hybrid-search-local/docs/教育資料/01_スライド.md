...
**対象**
`study-ml-foundations` の教材スライドを受講済みの IT エンジニア
...
`ml/data/preprocess/generate_property_embeddings.py`
...
`pipeline/services/embeddings/me5_embedding_service.py`
...
`pipeline/application/search_properties.py`
...
`pipeline/adapters/search/meilisearch_property_search_adapter.py`
...
`pipeline/services/search/query_filter_builder.py`
...
`pipeline/adapters/embeddings/me5_embedding_adapter.py`
...
`pipeline/services/embeddings/similarity_service.py`
...
`pipeline/adapters/ranking/lgbm_reranking_adapter.py`
...
| **Rank Fusion**（Phase 4 以降で導入） | lexical rank と semantic rank を**順位ベース**で事前に融合してから rerank | スケール差に頑健。候補統合段が明確に独立 | 融合式のハイパラ調整が必要 |
...
> 本 Phase は特徴量化で止めます。RRF は **Phase 4 以降で候補統合段として明示的に導入** されます。
...
`pipeline/services/ranking/lgbm_reranker.py`
...
- `pipeline/application/search_properties.py` を読む
- `property_features` と `property_embeddings` の生成元を追う
- デモでログがどう残るか確認する

> 次回は、Phase 3 の GCP 基盤（Cloud Run / BigQuery / Dataform / Terraform）へ進みます。

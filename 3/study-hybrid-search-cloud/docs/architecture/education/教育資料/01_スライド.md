...
paginate: true
header: "GCP-ML入門"
footer: "study-hybrid-search-cloud"
style: |
  section {
    font-size: 25px;
    padding: 44px 56px;
...
**対象**
`/home/ubuntu/repos/study-gcp-mlops/3/study-hybrid-search-cloud` を読む IT エンジニア
...
## Phase 3 → Phase 4 で変わる点

次講義（GCP-ML Vertex 編）へのブリッジ。**検索コア（Meilisearch + `VECTOR_SEARCH` + RRF + LambdaRank）は不変**、Port/Adapter の adapter だけ Vertex 実装に差し替える「差分工事」が主題。

| 軸 | Phase 3（本講義） | Phase 4（次講義） |
|---|---|---|
| 学習オーケ | Cloud Run Jobs (`training-job` / `embedding-job`) | **Vertex AI Pipelines (KFP) × 2** |
| rerank 推論 | `search-api` lifespan で `lgb.Booster` を GCS load | **Vertex Endpoint** (CPR container) |
| クエリ埋め込み | `search-api` lifespan で sentence-transformers load | **Vertex Endpoint** (CPR container) |
| Model 管理 | GCS パス + `mlops.training_runs` | **Vertex Model Registry**（+ dual write） |
| 特徴量管理 | Dataform のみ | 同 + **Vertex Feature Group** wrap |
| 監視 | Scheduled Query `mean_drift_sigma` | **Vertex Model Monitoring v2** |
| 実験管理 | W&B のみ | **Vertex Experiments + W&B** dual |
| ハイパラ探索 | 手動 | **Vertex Vizier**（オプション） |
| 再学習 subscriber | Eventarc → Cloud Run Job | Eventarc → **Cloud Function** |
| Cloud Run メモリ | 4Gi（encoder + booster 同居） | **2Gi**（外部化で軽量化） |
| SA 数 | 5 + 1 WIF | **9 + 1 WIF = 10** |
| Feature parity invariant | 5 ファイル | **6 ファイル**（Feature Group が追加） |

### 引き継がれる設計（変わらないもの）
- Meilisearch BM25 + BigQuery `VECTOR_SEARCH` + RRF の 3 段候補抽出
- LightGBM LambdaRank + NDCG 監視
- Dataform `property_features_daily` + assertions
- Pub/Sub → BQ Subscription のログ経路
- Port/Adapter + AST/parity テスト（adapter 追加だけで境界維持）
- rerank-free MVP の思想（`final_rank = lexical_rank` フォールバック）

> Phase 4 では「全部書き直す」のではなく「境界を守って差し替える」工事です。

...
``scripts/ci/check_layers.py``; this test module just wraps every entry of
``RULES`` in its own pytest case so a failure points cleanly at the
offending file. To add a rule, edit ``scripts/ci/check_layers.py::RULES``
...
from scripts.ci.check_layers import REPO_ROOT, RULES, find_violations

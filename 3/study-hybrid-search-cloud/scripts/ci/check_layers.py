...
# common/ — Port + pure-logic layer
    ...
    "app/app/ports/publisher.py": COMMON_ADAPTERS,
    "app/app/ports/retrain_queries.py": COMMON_ADAPTERS,
    "app/app/ports/training_job_runner.py": COMMON_ADAPTERS,
    "app/app/ports/cache_store.py": COMMON_ADAPTERS,
    "app/app/ports/lexical_search.py": COMMON_ADAPTERS,
    "app/app/ports/candidate_retriever.py": COMMON_ADAPTERS | frozenset({"lightgbm"}),
    "app/app/ports/model_store.py": COMMON_ADAPTERS | frozenset({"lightgbm"}),
    "app/app/services/retrain_policy.py": COMMON_ADAPTERS,
    "app/app/services/ranking.py": COMMON_ADAPTERS | frozenset({"sentence_transformers"}),
    "app/app/services/model_store.py": COMMON_ADAPTERS,
    "app/app/schemas/search.py": COMMON_ADAPTERS | frozenset({"lightgbm", "numpy"}),
    "app/app/api/middleware/request_logging.py": COMMON_ADAPTERS,
    "app/app/config.py": COMMON_ADAPTERS | frozenset({"lightgbm"}),
    # ml/training/ — pure training logic and Ports
    "ml/training/job.py": frozenset(),
    "ml/training/ports/ranker_repository.py": COMMON_ADAPTERS
    ...
    # ml/data/ — embedding and indexing orchestration
    "ml/data/job.py": frozenset(),
    "ml/data/preprocess/embedding_runner.py": COMMON_ADAPTERS
    | frozenset({"lightgbm", "ml.training", "train.adapters"}),
    "ml/data/datasets/embed_settings.py": frozenset({"lightgbm", "ml.training"}),
}

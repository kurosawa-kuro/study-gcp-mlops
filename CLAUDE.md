...
| 5 | `5/study-hybrid-search-vertex/` | 不動産検索 (Vertex AI) | Vertex AI プリミティブ化 (Pipelines / **Feature Store + Feature Group + Feature Online Store (training-serving skew 防止のため必須)** / **Vertex Vector Search (semantic 正本ストア)** / Model Registry / Monitoring v2)。Phase 4 の BigQuery `VECTOR_SEARCH` 経路を Vertex Vector Search に置換 |
...
- Phase 7 (Optional): GKE + KServe serving (学習側は Phase 5/6 の Vertex AI Pipelines + Vertex Vector Search を継承)。KServe から Feature Online Store を opt-in 参照する経路を追加

...
| 6 | `6/study-hybrid-search-pmle/` | GCP PMLE 試験範囲の実統合 | **Phase 5 完成系に PMLE 追加技術を adapter / 副経路 / 追加エンドポイント / 追加 Terraform として実統合** (BQML / Dataflow / Explainable AI / Monitoring SLO / Scheduled Query / **Cloud Composer (opt-in)** 等)。Feature Store / Vertex Vector Search は Phase 5 から継承(本 Phase では Dataflow / Scheduled Query で feature 入力生成・更新を強化)。Cloud Composer は Phase 6 限定の orchestration lab で、Cloud Scheduler 本線は維持。不変は「不動産ハイブリッド検索というテーマとその中核コード」のみ |
...
- Phase 6: Phase 5 完成系 + PMLE 追加技術統合 (BQML / Dataflow / Explainable AI / Monitoring SLO + **Cloud Composer (opt-in orchestration lab)** 等)。Feature Store と Vertex Vector Search は Phase 5 から継承。**Cloud Composer は Phase 6 限定の opt-in 副経路** (Cloud Scheduler + Eventarc + Cloud Function + Vertex Pipelines の本線は default 維持、Composer は DAG で束ねる比較教材)
- Phase 7 (到達ゴール): GKE + KServe serving (学習側は Phase 5/6 の Vertex AI Pipelines + Vertex Vector Search + Feature Store を継承)。KServe から Feature Online Store を opt-in 参照する経路を追加。**Cloud Composer は Phase 7 には継承しない** (Phase 6 限定の orchestration lab、Phase 7 は GKE/KServe 到達ゴールに集中)

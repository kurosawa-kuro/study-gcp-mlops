...
| 5 | `5/study-hybrid-search-vertex/` | 不動産検索 (Vertex AI 本番MLOps基盤) | Vertex AI 本番MLOps プリミティブ化 (Pipelines / **Feature Store + Feature Group + Feature Online Store (training-serving skew 防止のため必須、PII / user_id は扱わない)** / **Vertex Vector Search (semantic 検索の本番 serving index、embedding 生成履歴・メタデータは BigQuery 側に保持)** / Model Registry / Monitoring v2 / **Cloud Composer 本線 orchestration**)。Phase 4 の BigQuery `VECTOR_SEARCH` 経路を Vertex Vector Search に置換。**Phase 4 の Cloud Scheduler + Eventarc + Cloud Function trigger は本線 retrain schedule から外すが、軽量代替・smoke / manual trigger 用途として残す** (Vertex `PipelineJobSchedule` だけは併存禁止 = 完全撤去)。実装順序は **5A**(Pipelines/Registry/Endpoint) → **5B**(Feature Store) → **5C**(Vertex Vector Search) → **5D**(Composer DAG) → **5E**(Monitoring/Dataform) |
| 6 | `6/study-hybrid-search-pmle/` | GCP PMLE 試験範囲の実統合 | **Phase 5 完成系に PMLE 追加技術を adapter / 追加エンドポイント / 追加 Terraform として実統合** (BQML / Dataflow / Explainable AI / Monitoring SLO / Scheduled Query 等)。Cloud Composer は Phase 5 から継承し、PMLE 追加 DAG (Dataflow / BQML / drift query 等) を増設。Feature Store / Vertex Vector Search は Phase 5 から継承。不変は「不動産ハイブリッド検索というテーマとその中核コード」のみ |
| 7 | `7/study-hybrid-search-gke/` | 不動産検索 (GKE + KServe, 到達ゴール) | **Phase 6 の serving 層のみ** を GKE Deployment + KServe InferenceService に差し替え (Pipelines / **Vertex AI Feature Store + Feature Group + Feature Online Store** / **Vertex Vector Search** / **Cloud Composer orchestration (Phase 5 → 6 → 7 と継承)** / Model Registry / BigQuery / Meilisearch は Phase 6 から継承。Feature Online Store は Phase 5 構築済を KServe から参照) |
...
- **Cloud Composer (Phase 5 必須、Phase 6/7 継承)**: Managed Airflow Gen 3 を本線オーケストレーターとして Phase 5 で導入。本線 retrain schedule は Composer DAG (`daily_feature_refresh` / `retrain_orchestration` / `monitoring_validation` の 3 本起点)。**Vertex `PipelineJobSchedule` は完全撤去** (同一 PipelineJob 二重起動禁止)、**Cloud Scheduler / Eventarc / Cloud Function trigger は軽量代替・比較対象 / smoke / manual trigger 用途として Phase 5 以降も残す** (本線 retrain と同じ job を別系統で起動しないこと = 二重起動禁止)。Phase 6 で PMLE 追加 DAG (Dataflow / BQML / drift) を増設、Phase 7 はそのまま継承 (詳細は親 [`README.md` §「Cloud Composer の位置づけ」](README.md))
...
- Phase 4/5: クラウド実行基盤中心（Cloud Run / BigQuery / Vertex AI）。Phase 4 で BQ feature table / view の土台 + 軽量 serverless orchestration (Cloud Scheduler / Eventarc / Cloud Function) を作り、Phase 5 で Feature Store + Vertex Vector Search + **Cloud Composer 本線 orchestration** に格上げ (Composer DAG が Vertex Pipelines を submit、Cloud Scheduler / Eventarc / Cloud Function trigger / Vertex PipelineJobSchedule は Phase 5 で撤去 = Phase 4 → 5 の引き算境界)
- Phase 6: Phase 5 完成系 + PMLE 追加技術統合 (BQML / Dataflow / Explainable AI / Monitoring SLO 等)。Feature Store / Vertex Vector Search / Cloud Composer は Phase 5 から継承。Composer に PMLE 追加 DAG (Dataflow / BQML / drift) を増設
- Phase 7 (到達ゴール): GKE + KServe serving (学習側は Phase 5/6 の Vertex AI Pipelines + Vertex Vector Search + Feature Store + **Cloud Composer orchestration (Phase 5 → 6 → 7 と継承)** を継承)。KServe から Feature Online Store を opt-in 参照する経路を追加。**Composer DAG はそのまま継承し、orchestration 二重化を作らない**
...
cd 5/study-hybrid-search-vertex && make help    # Phase 4 継承 + Vertex AI + Cloud Composer 本線 orchestration (composer-deploy-dags 等)
cd 6/study-hybrid-search-pmle && make help    # Phase 5 継承 (Composer 含む) + PMLE 4 技術統合 (BQML / Dataflow / Explainable AI / Monitoring SLO) + Composer DAG 増設
